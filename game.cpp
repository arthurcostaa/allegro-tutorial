...
void fx_update() {
    for (int i = 0; i < FX_N; i++) {
        ...
        if (
            (!fx[i].spark && (fx[i].frame == (EXPLOSION_FRAMES * 2))) ||
...
void shots_draw() {
    for (int i = 0; i < SHOTS_N; i++) {
        ...
        if (shots[i].ship) {
            al_draw_bitmap(
                sprites.ship_shot[frame_display],
                shots[i].x, shots[i].y, 0
            );
        } else { // alien
            ALLEGRO_COLOR tint =
                frame_display
...
void ship_update() {
    ...
    if (ship.invincible_timer) ship.invincible_timer--;
    else {
    ...
    if (ship.shot_timer) ship.shot_timer--;
    else if (key[ALLEGRO_KEY_X]) {
        int x = ship.x + (SHIP_W / 2);
        if (shots_add(true, false, x, ship.y)) ship.shot_timer = 5;
    }
}
...
void aliens_update() {
    ...
    for (int i = 0; i < ALIENS_N; i++) {
        if (!aliens[i].used) {
            ...
            if (new_quota > 0) {
                new_x += between(40, 80);
                if (new_x > (BUFFER_W - 60)) new_x -= (BUFFER_W - 60);
...
void hud_update() {
    ...
    for (long i = 5; i > 0; i--) {
        long diff = 1 << i;
        if (score_display <= (score - diff)) score_display += diff;
    }
}
